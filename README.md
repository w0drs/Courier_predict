...
## Модель, метрика и функция потерь
### Постановка задачи
...
- MAPE "взрывается" при y_true = 0 (деление на ноль), а в задаче дефицита курьеров нулевые значения — реальная ситуация.  
- WAPE устойчива к выбросам и интерпретируема.  

### Функция потерь (LightGBM):
Вместо стандартных MAE/MSE использовал quantile loss (pinball loss).  
Почему:  
- задача дефицита требует асимметричного подхода — недооценка дефицита курьеров критичнее переоценки.  
- Quantile loss позволяет управлять этим через параметр alpha. 

### Модель 
Для задачи прогнозирования дефицита курьеров был выбран LightGBM по четырем причинам:
- Скорость — обучение быстрое, что позволило эффективно подбирать гиперпараметры через Optuna.
- Категориальные признаки — нативная поддержка без one-hot encoding.
- Память — нужно меньше RAM, чем у аналогов.
- Quantile loss — нативная поддержка асимметричной метрики через параметр objective='quantile'.

### Результаты различных моделей  
| Модель | WAPE | Комментарий |
|--------|------|-------------|
| Базовое предсказание (среднее) | 0.6 | Прогноз средним значением |  
| LightGBM (без feature engineering) | 0.3 | Только исходные признаки |  
| LightGBM (с feature engineering) | 0.1 | Итоговая модель |  

Итог: значительное улучшение относительно базового предсказания. 
